...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runningSpentCalories := (runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speed := meanSpeed(action, duration)
	walkingSpentCalories := (walkingCaloriesWeightMultiplier*weight + (math.Pow((speed*kmhInMsec), 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimmingSpentCalories := (speed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
